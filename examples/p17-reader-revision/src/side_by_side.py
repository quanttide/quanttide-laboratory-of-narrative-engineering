...
SIMULATE_AUTHOR_PROMPT = """你是一位了解作者写作契约的编辑。给定一个文本点，推断作者在这个位置上「想做什么」。
...
用一句话说清楚：作者在这里想达到什么效果？想通过这个细节表达什么？
不要说"作者想…"之类引导句，直接说出意图。

输出纯文本。"""


def simulate_author_intent(text_point, contracts, temperature=0.3) -> str:
    """LLM 推断作者在这个文本点上的创作意图。"""
    ...
    prompt = SIMULATE_AUTHOR_PROMPT.format(
        contracts=c,
        location=text_point["location"],
        quote=text_point["quote"],
    )
    raw = call_llm(prompt, system="你是一个编辑。只说推断，不评价。", temperature=temperature)
    return raw.strip().strip('"\'')


SYSTEM_PROMPT = (
    "你是一个材料整理工具。你的唯一任务是把三块信息并排呈现。\n"
...
PROMPT_TEMPLATE = """把以下三块信息并排呈现。只呈现材料，不做任何分析。
...
注意：
1. 作者原意部分：直接陈述推断，不要用"作者想"这类引导词。
2. 契约立场部分：直接引用条款原文，判断只陈述事实。
3. 读者回响部分：用模拟读者的话呈现，不要分析数据。
4. 绝对不能出现以下句式：
   - "这意味着……" / "这说明……"
   - "建议……" / "可以……" / "需要……"
   - "因此……" / "所以……"
...
def generate_side_by_side(text_point, contract_annotation, reader_response, profiles, contracts, temperature=0.3):
    ...
    # 模拟作者意图（LLM）
    author_intent = simulate_author_intent(text_point, contracts, temperature)

    # 模拟读者意见（LLM），保存在 reader_response 中供 HTML 使用
