...
| **p15** 情节结构的形式化与约束矩阵 | 合并 p12+p15。三类薄弱点界定 + 结构改法 4/6 优于表面改法 + 数学形式化尝试（负结论） + 约束矩阵检测 1 个异常 | 🟡 验收合格 |

## 创作建议

### 基于 p15：改结构，不是改措辞

p15 发现 LLM 能识别三种常见的结构薄弱点：因果跳跃（动机→行动→后果的链断了）、节奏失衡（事件密度不均）、动机不明（人物行为缺乏理由）。结构改法在 4/6 次上优于表面改法（"丰富细节""加强描写"）。
...
| p15 情节结构的形式化与约束矩阵 | 合并 p12→p15。三类薄弱点 + 4/6 结论 + 形式化尝试(负结论) + 约束矩阵 | [`p15-plot-math/`](p15-plot-math/) |
