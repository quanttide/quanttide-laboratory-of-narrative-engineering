...
| **p15** 情节结构的数学形式化 | （进行中）将 p12 的因果跳跃/节奏失衡/动机不明映射为因果图/密度函数/贝叶斯惊奇，验证文学诊断与数学度量的一致性 | 🟡 进行中 |
...
| p14 片段内部情节建议 | （进行中）诊断场景内行为因果链与情感递进 | [`p14-intra-scene-plot/`](p14-intra-scene-plot/) |
| p15 情节结构的数学形式化 | （进行中）因果图/密度函数/贝叶斯惊奇与 p12 诊断的一致性 | [`p15-plot-math/`](p15-plot-math/) |
