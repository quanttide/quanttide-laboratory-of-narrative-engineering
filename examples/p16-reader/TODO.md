...
> E4-5 的 ρ=-0.68 根因追踪结论：保留符号修复后 ρ 恢复到 0.097（方向对了但无预测力），ANOVA 通过（p=0.027）。所有画像的回归系数符号分布完全一致（n=6 自由度不足），放弃回归权重映射路径。

### R1 — 修复权重符号

- [x] **R1.1 修改 `src/layer2/weight_ratios.py`** — 去掉 LINE-77 的 `np.abs(w)`，保留原始回归系数
- [x] **R1.2 分析结果** — ρ=0.097 ❌ ICC=0.275 ❌ ANOVA=0.027 ✅
- [x] **R1.3 结论** — 回归权重映射在 n=6 下不可行，放弃此方案
