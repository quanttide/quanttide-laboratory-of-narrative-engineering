...
def report(similarity_results, reconstruction, blind_results):
    print("\n" + "=" * 60)
...
def main():
    ...
    from src.models import MotifSimilarityPair
    ...
    if full_fragments:
        ...
        print(f"\n  类数: {len(reconstruction.get('clusters', []))} → {len(full_reconstruction.get('clusters', []))}")
