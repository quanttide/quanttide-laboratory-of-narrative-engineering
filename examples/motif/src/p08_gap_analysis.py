...
from src.domain import DIRECTIONS
...
def main():
    ...
    for art in ARTICLES:
        ...
        gap_report = cache_or_compute(
            RESULTS_DIR / f"motif_report_{art['id']}.json",
            lambda: {**compute_gap_report(extract_motifs(text, art["name"]), target_motifs),
                     "extracted_motifs": extract_motifs(text, art["name"])["motifs"]},
            f"母题报告 {art['id']}",
        )
        ...
        for gap in gaps_to_fix:
            ...
            suggestions = cache_or_compute(
                ...
                lambda: generate_suggestions(art["name"], text, gap, attr.get("gap_types", []),
        ...
        for gt, elist in evals.items():
            ...
            for d in DIRECTIONS:
                ...
                if de:
                    ...
                    print(f"    {d['name']:<4}: feas={s.get('feasibility','?')} fit={s.get('motif_fit','?')} nat={s.get('naturalness','?')} act={s.get('actionable','?')} avg={avg:.1f}")
