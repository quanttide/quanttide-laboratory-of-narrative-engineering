...
## 建议类型
- 类型A 因果补链：因果链断裂时，补一个中间环节。适用：causal_gap、emotional_jump。
- 类型B 动机刻深：保留行为不变，注入情感动机。适用：boundary_shift、motivation_blind。
  fix_B 的质量判断：动机必须来自**具体的感官触发**（看到了什么/想起了什么/感受到了什么），
  不能来自抽象的社交理由（"怕她尴尬""想弥补退缩"）。好例子："看到她低头擦发梢，忽然想起十年前她淋雨的样子，心一软"。
- 类型C 替代行为：换一种不同的行为达到同样的情感效果，不是调远调近，而是"换一条路越界"。
  适用：boundary_shift。好例子：不直接点单，而是把菜单推过去说"这家焦糖玛奇朵不错——你看要不要试试"。
...
2. 每个薄弱点至少输出 2 条建议（适用的类型）。让创作者可以比较。
