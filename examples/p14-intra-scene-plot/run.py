...
## 建议类型（三种通用策略，适用于任何薄弱点类型）

类型A - 加一个中间节拍：在问题行为之前或之后，插入一个短暂的动作/停顿/眼神，让行为转换更自然。
  好例子：递毛巾后、点单前，他低头看自己的手，犹豫了一下，才开口。

类型B - 注入情感动机：保留行为不变，用具体的感官触发解释行为动机。必须基于"看到了/想起了/感受到了"，
  不能基于社交理由。行为的落点必须是**通用的**（问"喝点热的？"而不是精准命中她的偏好）。

类型C - 替代行为：换一种不同的行为达到同样的情感效果，不要"调远"（后退），要"换一条路靠近"。
  好例子：不直接点单，而是把菜单推过去说"他们家焦糖玛奇朵不错"——推荐店而非推荐她。

## 输出原则
每个薄弱点至少生成 2 条有内容的建议（如果该类型适用）。让创作者有选择余地。

## 重要约束
1. chain_status="有意断裂" 的节点不要生成建议——断裂是设计意图。
2. 每个薄弱点至少输出 2 条建议（类型A/B/C中任选适用的）。让创作者可以比较。
3. 类型B的动机必须基于感官触发（看到/听到/想到），不能基于社交计算（弥补退缩/怕尴尬）。行为落点必须通用（不能暴露"他知道她的喜好"）。
4. 类型C不要"调远"（后退），要"换一条路靠近"。这个故事的审美核心是靠近，不是后退。
...
## 作品的故事模型（来自 story.yaml，含 clue 和 tension）
{story_text}
