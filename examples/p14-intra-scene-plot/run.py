...
- 类型B 动机刻深：动机模糊时，改写行为让动机性格化。适用：motivation_blind、boundary_shift。
- 类型C 边界校准：社交距离不对时，调远或调近。适用：boundary_shift（仅当 fix_B 无法覆盖时）。

## 重要约束
1. chain_status="有意断裂" 的节点不要生成建议——断裂是设计意图。
2. 边界校准（类型C）**默认方向是调近**而非调远。这个故事的审美核心是"行为比社交身份更亲密"——人物总是在越过边界。如果 fix_B（动机刻深）已经生成了，fix_C 应为 null。不需要既补动机又调行为。
3. 调远的唯一适用场景：行为明显损害人物一致性（如初见面就做出恋人级别的亲密动作），且无法通过动机解释。对"焦糖玛奇朵"这类有十年暗恋背景的行为，调远是错的。
